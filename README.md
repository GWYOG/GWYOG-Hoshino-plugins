...
默认每人每天最多只能获取5张卡，想要修改数量请改代码前部的相关参数。
...
由于需使用`.ocr_image`API，请使用v0.9.26+的go-cqhttp。此外，请给机器人管理员权限，群主权限最佳。

新增：`STRICT MODE`: 由于不同手机分辨率不同和QQ压图的原因，有时并不能很好地识别出抽卡图中的文字“NEW”。为了防止漏鲨海豹，新增`STRICT MODE`，默认为开启状态，可在代码开头部分修改，开启后对于检测不出“NEW”的未达抽卡阈值的抽卡图，同样视为海豹图，不过不再禁言，只撤回。

目前还处于实验性阶段，由于插件会对群里发送的~~大部分图片~~（EDIT: 感谢HoshinoBot群友们提供的创意，通过进行文件大小过滤后，表情包已经基本完全被过滤掉了，现在只会对小部分图片OCR了，因此一直开着也无妨）进行文字识别，不清楚会不会因此产生风控之类的不可测后果(小规模测试尚未发现)。建议在小群使用，且建议只在开放新卡池的第一天使用。
